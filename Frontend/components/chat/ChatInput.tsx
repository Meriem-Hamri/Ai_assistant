...
interface ChatInputProps { isGenerating: boolean; onSend: (question: string) => Promise<void>; }

export function ChatInput({ isGenerating, onSend }: ChatInputProps) {
  const [question, setQuestion] = useState("");
  const canSend = question.trim().length > 0 && !isGenerating;
  ...
  return <div className="chat-input-container">
    <form className="chat-input-form" onSubmit={handleSubmit}>
      ...
      <textarea id="chat-question" className="chat-input" value={question} onChange={(event) => setQuestion(event.target.value)} onKeyDown={handleKeyDown} placeholder="Posez une question sur vos documents" rows={1} disabled={isGenerating} />
